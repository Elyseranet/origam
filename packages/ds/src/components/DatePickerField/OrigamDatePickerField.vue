...
>
	 ...
	 * SHADOWED the `validationValue` PROP (`IDatePickerFieldProps` ->
	 ...
	 * above, there was no second path either: a
	 * `<origam-date-picker-field :validation-value="somethingElse" />`
	...
	/*********************************************************
	 * toggleLabel / accessibleLabel (#666)
	 *
	 * @description
	 * This was previously named `label`, a bare `const` that SHADOWED the
	 * `label` PROP (`IDatePickerFieldProps` -> `ITextFieldProps` ->
	 ...
	 * `<origam-date-picker-field>` announced "Open"/"Close" to assistive
