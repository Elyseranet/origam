...
| `elevation`     | `string \| number`                                               | `undefined`   | Picks a shadow token rung (`'none'`, `'xs'`, `'sm'`, `'md'`, `'lg'`, `'xl'`) or a Material 0..24 level. ⚠️ `'2xl'` and `'3xl'` are accepted but render **exactly like `'xl'`** — no sheet declares their token, so they fall back to it (#813). For a heavier shadow, pass a free-form `box-shadow` string. |
