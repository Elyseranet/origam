...
`<OrigamMediaScrubber>` — see [Modes](#modes) below for the
...
> It switches between three structurally different renders (wrapped in
> `<origam-input>` chrome, or bare, or bare + waveform) — a behavioural
> discriminant, not a style preset — so it kept its own name instead of
> joining the props-preset model.

The `mode` prop drives three rendering paths.
...
### Mode & media-scrubber props (Phase 2)

| Prop | Type | Default | Description |
|---|---|---|---|
| `mode` | `'field' \| 'timer' \| 'audio'` | `'field'` | Usage mode. See [Modes](#modes). |
...
| `peaks` | `ReadonlyArray<number>` | — | Waveform peaks (each in `[0, 1]`). Painted as vertical SVG bars behind the track when `mode='audio'`. No effect for other modes. |
...
|---|---|---|
| `--origam-slider-field---border-radius` | `{radius.full}` (pill) | Themeable default corner radius for the surfaces owned directly by `OrigamSliderField` (the `buffered` fill, and the inset/bare-mode rail overrides). Overridden inline whenever the `rounded` prop (or `trackProps.rounded`) is set. |
...
| `--origam-slider-field--bare---thumb-size` | `12px` | Thumb diameter for `timer` / `audio` modes. |
...
| `--origam-slider-field--audio---track-height` | `48px` | Vertical room reserved for the waveform bars (`mode='audio'`). |
| `--origam-slider-field__buffered---background-color` | `color-mix(in srgb, currentColor 40%, transparent)` | Buffered-fill colour. |
| `--origam-slider-field__buffered---opacity` | `0.5` | Buffered-fill opacity (`mode='field'`). |
