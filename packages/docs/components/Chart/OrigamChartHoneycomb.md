...
| Name | Type | Default | Description |
|---|---|---|---|
| `xAxisFormat` | `(value: number \| string) => string` | `undefined` | ⛔ **Sans effet sur ce composant** — `labelFor` only ever applies `yAxisFormat`; a hex grid has no x axis. La prop reste declaree (elle est heritee d'`IChartBaseProps`) et emet un avertissement de developpement si elle est passee. Voir #426. |
