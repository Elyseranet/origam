// Regression for #381 — `const {id, ...} = useStyle(listStyles)` without
...
describe('OrigamList — consumer id reaches the root (#381)', () => {
    it('renders the consumer-supplied id on the root, not a generated one', () => {
        const wrapper = mount(OrigamList, {
            props: { id: 'my-list-id' } as never,
            global: { plugins: [createOrigam()] }
        })

        expect(wrapper.element.id).toBe('my-list-id')
        wrapper.unmount()
    })
})
