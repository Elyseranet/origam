...
//
// OrigamListItem declares `click` as its OWN emit. Vue strips any listener
...
// `link` prop, no group, no `value`. Confirmed empirically pre-fix:
// `Object.keys($attrs)` was EMPTY and `tabindex` was `undefined` on a list
// item mounted with only `onClick` — reachable by mouse (Vue's own
// emit-forwarding still invoked the handler) but invisible to Tab, and
// Enter/Space did nothing because `isClickable` never went true.
// ---------------------------------------------------------------------------

describe('OrigamListItem — isClickable via a plain @click, no `link` prop (#397)', () => {
    it('is focusable (tabindex=0) from a bare onClick listener alone', () => {
        const wrapper = mountListItem({ onClick: () => {} })
        expect(wrapper.find('.origam-list-item').attributes('tabindex')).toBe('0')
    })

    it('carries the --link modifier class from a bare onClick listener alone', () => {
        const wrapper = mountListItem({ onClick: () => {} })
        expect(wrapper.find('.origam-list-item').classes()).toContain('origam-list-item--link')
    })
