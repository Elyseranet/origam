// Regression for #381 — `const {id, ...} = useStyle(labelStyles)` without
...
describe('OrigamLabel — consumer id reaches the root (#381)', () => {
    it('renders the consumer-supplied id on the root, not a generated one', () => {
        const wrapper = mount(OrigamLabel, {
            props: { id: 'my-label-id' } as never,
            global: { plugins: [createOrigam()] }
        })

        expect(wrapper.element.id).toBe('my-label-id')
        wrapper.unmount()
    })
})
