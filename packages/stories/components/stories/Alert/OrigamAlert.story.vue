<template>
  ...
  >
    ...
    <Variant title="Events - update:modelValue"
             :init-state="() => useStoryInitState<IAlertProps>({ text: 'Dismiss this alert to fire update:modelValue.', closable: true, modelValue: true })"
    >
      <template #default="{state}">
        <origam-alert
    ...
    >
      <template #default="{ state }">
        <origam-alert v-bind="state" @click:close="logEvent('click:close', $event)"
                      @update:hover="logEvent('update:hover', $event)"/>
      </template>
      <template #controls="{ state }">
        <StoryGroup title="Content">
