...
- `pnpm -F origam guards` must stay green — **every** guard, not a fixed count.
  ⛔ Do not quote a number from this file: the total moves as guards are added.
  It read `17/17` here while the suite actually ran **23/23** (measured
  2026-09-16). Recount it on your own branch. If a change touches the token
